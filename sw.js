// ══════════════════════════════════════════════════════
//  sw.js — Service Worker لـ خزينتي (Khazinaty)
//  الإصدار: 1.0.0
// ══════════════════════════════════════════════════════

// غيّرنا اسم الكاش عشان النسخة الجديدة (تصميم الهيدر/الأيقونات الجديد) تتحمّل فريش، بدل
// ما تفضل عالقة على نسخة قديمة مخزّنة باسم الكاش القديم
const CACHE_NAME = 'khazinaty-v5';
...
self.addEventListener('fetch', (event) => {
  ...
  // ━━ style.css → Network First زي الـ HTML بالظبط (بيتغيّر كتير وقت التطوير، محتاج يوصل فريش دايمًا) ━━
      ...
      cache.match(request).then((cached) => {
        const networkFetch = fetch(request, { cache: 'no-store' }).then((response) => {
          if (response.ok && request.method === 'GET') {
            ...
            _bgtTrimCache(cache); // منع نمو الكاش بلا حدود مع الوقت
          }
          return response;
        }).catch(() => null);
...
// ── حماية بسيطة من تضخّم الكاش: حد أقصى لعدد العناصر المخزّنة ديناميكيًا ──
const MAX_DYNAMIC_CACHE_ENTRIES = 80;
async function _bgtTrimCache(cache) {
  try {
    const keys = await cache.keys();
    if (keys.length > MAX_DYNAMIC_CACHE_ENTRIES) {
      // بنمسح الأقدم (أول عناصر في القائمة) لحد ما نرجع تحت الحد
      const toDelete = keys.length - MAX_DYNAMIC_CACHE_ENTRIES;
